...
class _FakeClient:
    """Stands in for FdcClient, returning a canned raw search payload."""
...
async def test_malformed_barcode_short_circuits_without_calling_upstream(fake_fdc):
    ...
    assert client.requests == []  # never hit the API


async def test_queries_branded_foods_search_endpoint(fake_fdc):
    client = fake_fdc([{"fdcId": 888, "gtinUpc": "028400642255"}])
    await usda_fdc.search_by_upc("028400642255")
    endpoint, params = client.requests[0]
    assert endpoint == "foods/search"
    assert params["query"] == "028400642255"
