...
This first pass reaches a modest 4.4% of real tag-occurrences — sized to what one careful verification pass actually produced, not a target chosen in advance, the same incremental spirit as FDC's own multi-round path to 91.4%. Broader coverage is future work, the same round-over-round pattern.
