...
**The fix treats the two sources differently, because they have different shapes.** FDC's `branded_food_category` is a **closed, controlled vocabulary** — 350 distinct values across the whole corpus, GDSN-standardised, Pareto-distributed (the top 20 alone cover 48.7% of all branded foods; the top 90 cover 90.7%). That is small and stable enough to **hand-verify**, so `app/core/gpc_match.py` carries two curated tables — each entry looked up and read against the real GPC taxonomy, not matched by string similarity — covering **87.0%** of all FDC foods with a category:
